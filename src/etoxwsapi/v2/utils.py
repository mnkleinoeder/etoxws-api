...
class JobStat(object):
    ...
    def is_open(self):
        return (self.is_accepted() or self.is_running() or self.stat['status'] == "JOB_UNKNOWN")

    def is_done(self):
        return (self.stat['status'] in ( "JOB_COMPLETED", "JOB_FAILED", "JOB_REJECTED", "JOB_CANCELLED"))
